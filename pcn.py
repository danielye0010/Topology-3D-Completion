...
SAVE_DIR = os.getenv(
    "TOPO_PCN_SAVE_DIR", str(REPO_ROOT / "runs" / "plane_pcn_baseline")
)
...
class PlaneDS(Dataset):
    """Load paired incomplete/clean clouds in one shared coordinate frame."""

    def __init__(self, pairs, n_in, train=True):
        self.pairs = pairs
        self.n_in = n_in
        self.train = train

    @staticmethod
    def _load_points(fp, n):
    ...
    def _normalize_pair(x, y):
        # Use the incomplete cloud's frame so the same normalization is available
        # at inference time, then apply it identically to the clean target.
    ...
    def _augment_pair(x, y):
        ...
        # Input-only jitter simulates acquisition noise without moving the target.
    ...
    def __getitem__(self, i):
        ...
        x = self._load_points(drop_fp, self.n_in)
        y = self._load_points(clean_fp, self.n_in * 4)
...
def fscore(a, b, thr=F_THRESH):
    d1 = torch.cdist(a, b).min(-1)[0]
    d2 = torch.cdist(b, a).min(-1)[0]
    p = (d1 < thr).float().mean()
    r = (d2 < thr).float().mean()
...
class PCN(nn.Module):
    ...
    def forward(self, x):
        ...
        feat = torch.cat([f, g], -1)
        feat = self.enc2(feat).max(1)[0]

        coarse = self.dec_c(feat).view(B, self.nc, 3).clamp(-1, 1)

        grid = self.grid[None, None].expand(B, self.nc, -1, -1)
        center = coarse[:, :, None, :].expand(-1, -1, self.g**2, -1)
        gfeat = feat[:, None, None, :].expand(-1, self.nc, self.g**2, -1)
        foldin = torch.cat([grid, center, gfeat], -1)
        fine = (self.fold(foldin) + center).view(B, -1, 3).clamp(-1, 1)
        return coarse, fine
...
def evaluate(model, loader):
    model.eval()
    cd_tot = hole_tot = pts_tot = fs_tot = 0.0
    for x, y in loader:
        ...
        for b in range(B):
            ...
            if len(idx):
                perm = torch.randperm(len(idx), device=idx.device)[:MAX_HOLE_PTS]
                idx = idx[perm]
                hole_tot += (
                    chamfer(pred[b : b + 1], y[b][idx][None]).item() * len(idx)
                )
                pts_tot += len(idx)
...
def main():
    ...
    model.load_state_dict(
        torch.load(os.path.join(SAVE_DIR, "best_model_pcn.pth"), map_location=DEVICE)
    )
